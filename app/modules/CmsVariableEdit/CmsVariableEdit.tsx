      ...
      <Query query={QUERY_GLOBAL_CMS_VARIABLE} variables={{ id: params.id }}>
        {({ loading, data: { globalCmsVariable } }) => {
          ...
          return (
            <div>
              <Mutation
                mutation={MUTATION_UPDATE_CMS_VARIABLE}
                update={(store, { data: { updateCmsVariable } }) => {
                  message.success(`CMS Variable ${updateCmsVariable.label} has been updated`);

                  const cmsVariablesQuery: any = store.readQuery({
                    query: QUERY_GLOBAL_CMS_VARIABLES,
                  });
                  const cmsVariables = cmsVariablesQuery
                    ? cmsVariablesQuery.globalCmsVariables
                    : [];
                  const index = cmsVariables.findIndex(
                    (cmsVariable: any) => cmsVariable.id == updateCmsVariable.id
                  );
                  cmsVariables[index] = updateCmsVariable;
                  store.writeQuery({
                    query: QUERY_GLOBAL_CMS_VARIABLES,
                    data: { globalCmsVariables: cmsVariables },
                  });
                }}
              >
                {updateCmsVariable => (
